...
function BdContactCard({ facility }: { facility: Facility }) {
  const navigate = useNavigate();
  const { user } = useAuth();
  const hasContact = facility.bd_contact_name || facility.bd_contact_phone || facility.bd_contact_email;

  const handleMessage = async () => {
    if (!user) { navigate("/login"); return; }
    // Try to find a profile by BD contact email
    if (facility.bd_contact_email) {
      const { data } = await supabase
        .from("profiles")
        .select("user_id")
        .ilike("email", facility.bd_contact_email)
        .maybeSingle();
      if (data?.user_id && data.user_id !== user.id) {
        navigate(`/app/messages?to=${data.user_id}`);
        return;
      }
    }
    ...
    if (members && members.length > 0) {
      navigate(`/app/messages?to=${members[0].user_id}`);
      return;
    }
    toast.error("This BD rep hasn't joined the platform yet.");
  };
  ...
  return (
    <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-accent/30 to-card shadow-md overflow-hidden text-center">
      <div className="bg-gradient-to-r from-primary to-primary/80 text-primary-foreground px-5 py-4">
        <p className="font-heading font-bold text-lg uppercase tracking-wider">For Referrals</p>
      </div>
        ...
        {hasContact ? (
          <>
            {facility.bd_contact_name && (
              <div>
                <p className="text-xs uppercase font-semibold tracking-wider text-muted-foreground mb-1">Contact</p>
                <p className="font-semibold">{facility.bd_contact_name}</p>
              </div>
            )}
            <Button size="lg" className="w-full shadow-sm" onClick={handleMessage}>
              <MessageSquare className="h-4 w-4" /> Send a message
            </Button>
            {facility.bd_contact_email && (
              <a href={`mailto:${facility.bd_contact_email}`} className="block">
                <Button size="lg" variant="outline" className="w-full">
            ...
            {facility.bd_contact_phone && (
              <a href={`tel:${facility.bd_contact_phone}`} className="block">
                <Button size="lg" variant="outline" className="w-full">
          ...
          <p className="text-sm text-muted-foreground">No BD contact has been set for this facility yet.</p>
        )}
      </div>
    </div>
  );
