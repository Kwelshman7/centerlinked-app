...
function AnswerCard({
  ...
  return (
    ...
    >
      <article
        className={cn(
          "flex h-full min-h-[5.25rem] flex-col rounded-xl border border-border/40 bg-card/95 backdrop-blur-[2px]",
          "px-3 py-3 sm:px-3.5 sm:py-3.5",
          "shadow-[0_8px_22px_-14px_rgba(15,23,42,0.24)]",
          "transition-[transform,box-shadow] duration-300",
          "hover:-translate-y-0.5 hover:shadow-[0_14px_28px_-14px_rgba(15,23,42,0.28)]",
        )}
      >
        <div className="flex flex-1 items-start gap-2.5">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm shadow-primary/25">
            <Icon className="h-4 w-4" aria-hidden strokeWidth={2.25} />
          </span>
          <div className="min-w-0 flex-1 pt-0.5">
            <h3 className="font-display text-[14px] sm:text-[15px] font-semibold text-foreground leading-snug tracking-tight">
              <span>{before} </span>
...
export function LinkAnswersReveal({
  ...
  return (
    <div ref={ref} className={cn("space-y-5 sm:space-y-6", className)}>
      <ul className="grid grid-cols-1 auto-rows-fr gap-2.5 sm:grid-cols-2 sm:gap-2.5">
