 ...
 */
export function OrgHeroSection({ org, brand }: Props) {
  ...
  if (logoAsHero && heroImage) {
    return (
      ...
      >
        <div className="flex items-center justify-center py-1.5 sm:py-10 lg:py-14">
          <img
            src={heroImage}
            alt={org.name}
            className="h-[4.75rem] w-auto max-w-[88%] object-contain sm:h-28 sm:max-w-[55%] lg:h-36"
          />
        </div>
      </section>
    );
  ...
  return (
    <section className="relative w-full overflow-hidden bg-muted/40">
      <div className="relative w-full min-h-[160px] sm:min-h-[280px] lg:min-h-[400px]">
        {heroImage ? (
          <img
            src={heroImage}
            alt=""
