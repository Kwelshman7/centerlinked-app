...
function MetaColumn({
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-1 min-w-[9.5rem] sm:min-w-0 shrink-0 sm:shrink px-4 py-3.5 sm:py-4 snap-start border-r border-border/50 last:border-r-0",
        className,
      )}
    >
...
function FacilityPreviewCard({
  ...
  return (
    <div className="w-full lg:w-[17.5rem] xl:w-[18.5rem] lg:shrink-0 rounded-xl border border-border/60 bg-card overflow-hidden shadow-sm flex flex-col">
      ...
      <div className="p-3.5 sm:p-4 flex flex-col gap-2.5 flex-1">
        <div>
          <p className="font-heading font-bold text-sm sm:text-[15px] leading-snug break-words">{facility.name}</p>
          {loc && (
...
export function OrganizationSheetView({
  ...
  const tabs: { id: OrgTab; label: string; shortLabel?: string; count?: number }[] = [
    { id: "overview", label: "Overview" },
    { id: "facilities", label: "Facilities", count: facilities.length },
    { id: "services", label: "Services & Programs", shortLabel: "Services" },
    { id: "insurance", label: "Insurance" },
    { id: "accreditations", label: "Accreditations", shortLabel: "Accred." },
  ...
  const sidebar = (
    <div className="space-y-4 sm:space-y-5 lg:sticky lg:top-[4.5rem]">
      {heroContact ? (
        <OrgHeroContactCard
  ...
  return (
    <div className={cn("space-y-0", hasContact ? mobileContactBarPadding() : "")}>
      {/* Hero */}
      <section className="relative min-h-[220px] sm:min-h-[280px] lg:min-h-[320px] flex flex-col justify-end overflow-hidden rounded-xl sm:rounded-2xl border border-border/60 shadow-sm">
        ...
        <div className="absolute top-3 sm:top-4 left-3 sm:left-4 right-3 sm:right-4 z-10 flex flex-wrap items-start justify-between gap-2">
          {hasInNetwork && (
            <span
          ...
          {org.verified && (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/25 px-2.5 py-1 text-[10px] sm:text-[11px] font-semibold text-white shadow-sm ml-auto">
              <BadgeCheck className="h-3.5 w-3.5 shrink-0" />
              <span>Verified</span>
              {lastUpdated && (
                <>
                  <span className="h-3 w-px bg-white/40 hidden sm:inline" />
                  <span className="font-medium text-white/85 hidden sm:inline">Updated {lastUpdated}</span>
                </>
              )}
            </div>
          )}
        ...
        <div className="relative z-[1] px-4 sm:px-6 lg:px-8 py-5 sm:py-6 lg:py-8 pt-14 sm:pt-16">
          <h1
            className="font-heading text-xl sm:text-3xl lg:text-4xl font-bold tracking-tight leading-[1.15] text-white max-w-3xl break-words"
            style={{ textShadow: "0 2px 8px rgba(0,0,0,0.35)" }}
          >
            {org.name}
          </h1>

          <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5 mt-2.5 sm:mt-3 text-xs sm:text-sm text-white/90">
            {primaryAccreditation && (
              <span className="inline-flex items-center gap-1 max-w-full">
                <Award className="h-3.5 w-3.5 shrink-0" />
                <span className="break-words">{primaryAccreditation}</span>
              </span>
            )}
            {primaryAccreditation && loc && <span className="text-white/50 hidden sm:inline">•</span>}
            {loc && (
          ...
          {org.slug && (
            <div className="mt-4 sm:mt-5">
              <ShareSheetButton
                ...
                hideCopy
                className="w-full sm:w-auto shadow-sm hover:opacity-90"
      ...
      {/* Quick info bar */}
      <section className="mt-3 sm:mt-4 rounded-xl border border-border/60 bg-card shadow-sm overflow-hidden">
        <div className="flex overflow-x-auto sm:grid sm:grid-cols-3 lg:grid-cols-5 sm:overflow-visible snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {loc && <MetaColumn icon={MapPin} label="Headquarters" value={loc} brand={brand} className="pl-4" />}
      ...
      {/* Content tabs */}
      <div className="mt-5 sm:mt-6 sm:sticky sm:top-16 sm:z-20 -mx-4 px-4 sm:mx-0 sm:px-0 bg-muted/30 sm:bg-muted/80 sm:backdrop-blur-sm border-b border-border/60">
        <div
          role="tablist"
          className="flex items-center gap-0 overflow-x-auto snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "shrink-0 snap-start px-3 sm:px-4 py-3 sm:py-3.5 text-xs sm:text-sm font-medium whitespace-nowrap border-b-2 transition-colors -mb-px min-h-[44px]",
      ...
      {/* Main layout */}
      <div className="mt-5 sm:mt-6 grid lg:grid-cols-[minmax(0,1fr)_300px] xl:grid-cols-[minmax(0,1fr)_320px] gap-5 sm:gap-6 lg:gap-8 items-start">
          ...
          {activeTab === "overview" && (
            <>
              {previewFacilities.length > 0 && (
                <section>
                  <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2 sm:gap-4 mb-4">
                    ...
                    {facilities.length > previewFacilities.length && (
                      <button
                        ...
                        className="text-sm font-semibold hover:underline shrink-0 text-left"
                        style={{ color: brand }}
                      >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:hidden">
                    {previewFacilities.slice(0, 4).map((f) => (
                      <FacilityPreviewCard
                        key={f.id}
                        facility={f}
                        orgSlug={org.slug}
                  ...
                  <div className="hidden lg:flex gap-5 overflow-x-auto pb-2 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              <section>
                ...
                <div className="grid gap-5 md:grid-cols-[1fr_minmax(0,220px)] lg:grid-cols-1 xl:grid-cols-[1fr_minmax(0,240px)]">
                  <div className="min-w-0">
                  ...
                  {org.why_refer.length > 0 && (
                    <ul className="space-y-3 md:min-w-0">
                      {org.why_refer.slice(0, 3).map((item) => (
      ...
      <aside className="lg:hidden mt-5 sm:mt-6">{sidebar}</aside>

      <div className="mt-5 sm:mt-6 pb-6 sm:pb-8">
        <OrgFooter
          orgId={org.id}
          orgName={org.name}
          slug={org.slug}
