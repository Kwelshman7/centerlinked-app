        ...
        <div className="min-w-0 space-y-5">
          <section className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_rgba(15,23,42,0.04)]">
            <div className="space-y-5 p-4 sm:p-6">
                ...
                <div className="min-w-0 flex-1 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {profile.job_title ? <p className="text-sm font-medium text-foreground/80">{profile.job_title}</p> : null}
                    ...
                    {profile.organization?.logo_url ? (
                      <img
                        ...
                        alt=""
                        className="hidden h-12 w-12 rounded-xl bg-background object-contain p-1 ring-1 ring-border sm:block"
                      />
                    ) : null}
                  </div>
                  {bio ? (
                    <p className="text-sm leading-relaxed text-foreground/80">
                      <span className="line-clamp-3">{bio}</span>
                      {bio.length > 140 ? (
                        <button
                          type="button"
              ...
              <dl className="grid grid-cols-2 gap-3 border-t border-border/70 pt-4 sm:grid-cols-4">
          ...
          <Tabs value={tab} onValueChange={(value) => setTab(value as TabId)} className="space-y-4">
...
function Stat({ value, label }: { value: number; label: string }) {
  return (
    <div>
      ...
      <dd className="mt-0.5 font-heading text-xl font-bold tabular-nums">{value.toLocaleString("en-US")}</dd>
    </div>
  );
}
  ...
  return (
    <section className="space-y-3">
      ...
      {shown.length === 0 ? (
        ...
        <div className={cn("grid gap-4", shown.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 xl:grid-cols-3")}>
          {shown.map((facility) => (
            <ProfileFacilityCard
              key={facility.id}
              facility={facility}
