...
import {
  ...
  shortenLevelOfCare,
  uniquePreserve,
  type OrgPrintDensity,
  type OrgPrintPageKind,
} from "@/lib/org-one-pager-layout";

export type { OrgPrintDensity, OrgPrintPageKind };
export { densityForFacilityCount, paginateOrgFacilities, shortenLevelOfCare };
...
export function buildOrgOnePagerModel(input: BuildInput): OrgOnePagerModel {
  ...
  const mapped: OrgOnePagerFacility[] = visible.map((facility) => {
    ...
    return {
      ...
      whoWeTreat: pickWhoWeTreat(
        ...
        density === "generous" ? 5 : 0,
      ),
      specialties: uniquePreserve([...tags.therapies, ...tags.conditions]).slice(
        0,
        density === "generous" ? 5 : 0,
      ),
      payers,
  ...
  const payerSets = visible.map((f) => uniquePreserve(payersFrom(input.facilityPayersById, f.id)));
  const sharedAll = payerSetsAreShared(payerSets) ? payerSets[0] : null;
  const sharedPayers = sharedAll ? sharedAll.slice(0, cap) : null;

  const slices = paginateOrgFacilities(mapped.length);
  const pages: OrgOnePagerPage[] = slices.map((slice, index) => ({
    kind: slice.kind,
    pageNumber: index + 1,
    pageCount: slices.length,
    facilities: mapped.slice(slice.start, slice.end),
  }));
  ...
  const overview =
    condenseText(input.overviewOverride, 360) ||
    condenseText(input.org.description, density === "directory" ? 320 : density === "standard" ? 220 : 260);
