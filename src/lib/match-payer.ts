export interface PayerMatchInput {
  id: string;
  name: string;
  aliases?: string[] | null;
}

/** Lowercase, collapse whitespace, strip most punctuation for comparison. */
...
export function normalizeCompact(raw: string): string {
  return normalizePayerName(raw).replace(/\s+/g, "");
}

/** Contract labels that are not real payers — skip linking. */
export function isNonPayerLabel(name: string): boolean {
 ...
 */
export const CONTRACT_TO_PAYER: Record<string, string> = {
  ...
  "new york state empire plan": "Empire Blue Cross Blue Shield",
  "empire plan": "Empire Blue Cross Blue Shield",
  "va community care network": "VA Community Care Network",
  "va community care": "VA Community Care Network",
  "veteran community care": "VA Community Care Network",
  "veteran affairs community care network": "VA Community Care Network",
  "veteran affairs community care network vaccn": "VA Community Care Network",
  vaccn: "VA Community Care Network",
  "va ccn": "VA Community Care Network",
  "optum va ccn": "Optum VA CCN",
  optimserve: "Optum VA CCN",
  optumserve: "Optum VA CCN",
  va: "VA Community Care Network",
  lucet: "Carelon Behavioral Health",
  "comp psych": "ComPsych",
  comppsych: "ComPsych",
  "med mutual": "Medical Mutual of Ohio",
  "ambetter health": "Ambetter (Centene)",
  "pacific source navigator": "PacificSource Health Plans",
  "pacific source voyager": "PacificSource Health Plans",
...
export function contractNameVariants(payerName: string): string[] {
...
export function getPayerMatchTerms(payer: Pick<PayerMatchInput, "name" | "aliases">): string[] {
...
export function findPayerByName(name: string, payers: PayerMatchInput[]): PayerMatchInput | null {
...
export function resolvePayerForContractName(
  payerName: string,
  ...
  for (const variant of contractNameVariants(trimmed)) {
    const mapped = CONTRACT_TO_PAYER[normalizePayerName(variant)];
    if (mapped) {
...
/** PostgREST `.or()` filter for payer_id + payer_name variants (incl. aliases). */
export function buildPayerOrFilter(payer: PayerMatchInput): string {
  const parts = [`payer_id.eq.${payer.id}`];
  ...
  for (const raw of [payer.name, ...(payer.aliases ?? [])]) {
    ...
    seen.add(key);
    parts.push(`payer_name.ilike.${escapeOrFilterValue(term)}`);
    parts.push(`payer_name.ilike.${escapeOrFilterValue(`%${term}%`)}`);
  }
...
export interface InsuranceContractInsert {
  facility_id: string;
...
export function buildInsuranceContractRows(
  facilityId: string,
  payerNames: string[],
  inNetwork: boolean,
  payers: PayerMatchInput[],
): InsuranceContractInsert[] {
  return payerNames
    .map((raw) => raw.trim())
    .filter(Boolean)
    .map((rawName) => {
      const resolved = resolvePayerForContractName(rawName, payers);
      return {
        facility_id: facilityId,
        payer_id: resolved?.id ?? null,
        payer_name: resolved?.name ?? rawName,
